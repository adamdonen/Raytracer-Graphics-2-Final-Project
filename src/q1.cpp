...
const char *WINDOW_TITLE = "Ray Tracing Project";
...
void init(char *fn) {
	choose_scene(fn, theScene, useCamera);
	...
	if (drawing_y <= 0.5) {
		...
		if (drawing_y == int(drawing_y)) {
			...
			for (int x = 0; x < vp_width; x++) {
				if (useCamera) {
					if (!newTrace(theScene.camera->generateRay(s(x, y).x, s(x, y).y), texture[x], false)) {
						texture[x] = background_colour;
					}
				}
				else {

					if (!newTrace(new Ray(eye, s(x, y)), texture[x], false)) {
						texture[x] = background_colour;
					}
				}
			}
...
void mouse( int button, int state, int x, int y ) {
	...
	if ( state == GLUT_DOWN ) {
		...
		case GLUT_LEFT_BUTTON:
			...
			point3 uvw = s(x, y);
			std::cout << std::endl;
			if (newTrace(new Ray(eye, uvw), c, true)) {
				std::cout << "\nHIT @ ( " << uvw.x << "," << uvw.y << "," << uvw.z << " )\n";
